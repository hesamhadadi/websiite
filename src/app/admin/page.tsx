...
import { FolderKanban, FileText, MessageSquare, ArrowUpRight, BadgeCheck } from "lucide-react";
...
export default function AdminDashboard() {
  const [stats, setStats] = useState<Stats>({ projects: 0, posts: 0, messages: 0, unread: 0, certificates: 0 });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    async function fetchStats() {
      try {
        const [projectsRes, postsRes, messagesRes, certificatesRes] = await Promise.all([
          fetch("/api/projects"),
          fetch("/api/blog?all=true"),
          fetch("/api/messages"),
          fetch("/api/certificates"),
        ]);
        const [projects, posts, messages, certificates] = await Promise.all([
          projectsRes.json(),
          postsRes.json(),
          messagesRes.json(),
  ...
  const cards = [
    ...
    { label: "Certificates", value: stats.certificates, icon: BadgeCheck, href: "/admin/certificates", note: "Published" },
    ...
    <div>
      ...
      <div className="border border-border p-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Link href="/admin/certificates" className="px-4 py-2 border border-border font-mono text-xs text-text-secondary hover:text-accent hover:border-accent/30 transition-colors tracking-wider uppercase">
            + Add Certificate
          </Link>
